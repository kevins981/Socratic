...
export async function POST(req) {
  try {
    ...
    synthesizeSessionStore.appendLog(sessionId, `[INFO] Add concept session started. Launching: python3 ${args.join(' ')}`);
    ...
    return NextResponse.json({ error: err?.message || 'Failed to start add concept' }, { status: 500 });
  }
}
