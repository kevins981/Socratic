...
export async function POST(req) {
  try {
    ...
    const args = [
      ...
      '-m', 'socratic.cli', 'triage', // Always use triage mode
      '--project', projectName,
    ...
    const sessionId = triageSessionStore.createSession(child);
    ...
    const handleData = (chunk, isErr = false) => {
      ...
      text.split('\n').forEach((line, idx, arr) => {
        ...
        if (actual !== undefined) {
          ...
          triageSessionStore.appendLog(sessionId, prefixed);
        }
      });
    };
    ...
    child.on('exit', (code) => {
      triageSessionStore.setStatus(sessionId, 'exited', code ?? 0);
    });
    child.on('error', (err) => {
      triageSessionStore.appendLog(sessionId, `[ERR] ${err?.message || 'Process error'}`);
      triageSessionStore.setStatus(sessionId, 'error', null);
    });

    return NextResponse.json({ sessionId });
  } catch (err) {
    return NextResponse.json({ error: err?.message || 'Failed to start triage' }, { status: 500 });
  }
}
